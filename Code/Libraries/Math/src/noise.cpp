...
float Noise::SumNoise1( float x, const uint Octaves, Noise1Func NoiseFunc )
{
	float	Sum			= 0.0f;
...
float Noise::SumNoise2( float x, float y, const uint Octaves, Noise2Func NoiseFunc )
{
	float	Sum			= 0.0f;
	float	NoiseScale	= 1.0f;
...
float Noise::SumNoise3( float x, float y, float z, const uint Octaves, Noise3Func NoiseFunc )
{
	float	Sum			= 0.0f;
	float	NoiseScale	= 1.0f;
...
float Noise::GetOctaveScale( const uint Octaves )
{
	float	Sum		= 0.0f;
	float	Scale	= 1.0f;

	// Octave control vars--expose to client?
	static const float OctaveWeightRatio = 0.5f;
...
float Noise::SumAbsNoise1( float x, const uint Octaves, Noise1Func NoiseFunc )
{
	float	Sum			= 0.0f;
	float	NoiseScale	= 1.0f;
...
float Noise::SumAbsNoise2( float x, float y, const uint Octaves, Noise2Func NoiseFunc )
{
	float Sum = 0.0f;
	float NoiseScale = 1.0f;
...
float Noise::SumAbsNoise3( float x, float y, float z, const uint Octaves, Noise3Func NoiseFunc )
{
	float	Sum			= 0.0f;
	float	NoiseScale	= 1.0f;
...
static float Grad1( const int hash, const float x )
{
	const int	h	= hash & 15;
	const float	u	= x * ( 1.0f + ( h & 7 ) );
	return ( ( h & 8 ) ? -u : u );
}
...
static float Grad2( const int hash, const float x, const float y )
{
...
static float Grad3( const int hash, const float x, const float y,const  float z )
{
