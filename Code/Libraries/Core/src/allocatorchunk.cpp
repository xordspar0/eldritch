...
void AllocatorChunk::Report( const IDataStream& Stream ) const
{
	...
	const uint CurrentUsed	= m_AllocSpace + m_Overhead;
	const uint MaxUsed		= m_MaxAlloc + m_MaxOverhead;
	...
	Stream.PrintF( "\tCurrent used:\t\t%d (%f%%)\n",	CurrentUsed,	static_cast<float>( CurrentUsed ) * PercentSize );
	...
	Stream.PrintF( "\tMaximum used:\t\t%d (%f%%)\n",	MaxUsed,		static_cast<float>( MaxUsed ) * PercentSize );
...
void AllocatorChunk::ReportMemoryLeaks( const IDataStream& Stream, const uint Bookmark1, const uint Bookmark2 ) const
{
	...
	for( const SAllocatorBlock* pBlock = m_Head; pBlock; pBlock = pBlock->m_Next )
	{
		if( pBlock->m_Used && pBlock->m_AllocNum >= Bookmark1 && pBlock->m_AllocNum < Bookmark2 )
		{
			...
			// Left-justified string print, minimum 16 characters (padded), maximum 16 characters
			Stream.PrintF( "(\"%-16.16s\")\n", reinterpret_cast<const char*>( pBlock ) + Allocator::m_BlockSize );
		}
	}
#endif // BUILD_DEV
}
