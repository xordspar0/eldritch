...
namespace SoLoud
{
	...
	void WavStreamInstance::getAudio(float *aBuffer, unsigned int aSamples)
	{
		...
		if (mOgg)
		{
			...
			while (offset < aSamples)
			{
				...
				offset += b;
				mOggFrameOffset += b;
				// DLP 9 Feb 2019...: Fix a hang in Eldritch music
	...
	time WavStreamInstance::getLength()
	{
...
#define MAKEDWORD(a,b,c,d) (((d) << 24) | ((c) << 16) | ((b) << 8) | (a))
	...
	{
		// DLP 22 Aug 2018: optimize Vorbis loads
		if (mCachedChannels > 0)
		{
			mChannels = mCachedChannels;
			mBaseSamplerate = mCachedSampleRate;
			mSampleCount = mCachedSampleCount;
			mOgg = 1;
			return 0;
		}
