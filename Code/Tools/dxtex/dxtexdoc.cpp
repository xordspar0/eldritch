...
// DLP extension
// Create an A8R8G8B8 surface, swizzle R and A
void CDxtexDoc::DoNormalHack()
{
	HRESULT hr;
	D3DSURFACE_DESC sd;
	LPDIRECT3DDEVICE9 pd3ddev = PDxtexApp()->Pd3ddev();
	LPDIRECT3DTEXTURE9 pmiptex;
	LPDIRECT3DSURFACE9 psurfOrig;
	LPDIRECT3DSURFACE9 psurfSwizzle;

	EnsureAlpha(&m_ptexOrig);

	pmiptex = (LPDIRECT3DTEXTURE9)m_ptexOrig;
	hr = pmiptex->GetSurfaceLevel(0, &psurfOrig);
	psurfOrig->GetDesc(&sd);

	// Copy the target surface into an A8R8G8B8 surface
	hr = pd3ddev->CreateOffscreenPlainSurface(sd.Width, sd.Height, D3DFMT_A8R8G8B8, D3DPOOL_SCRATCH, &psurfSwizzle, NULL);
	hr = D3DXLoadSurfaceFromSurface(psurfSwizzle, NULL, NULL, psurfOrig, NULL, NULL, D3DX_DEFAULT, 0);

	D3DLOCKED_RECT lr;
	hr = psurfSwizzle->LockRect(&lr, NULL, 0);
	...
	for (yp = 0; yp < sd.Height; yp++)
	{
		...
		for (xp = 0; xp < sd.Width; xp++)
		{
			// Move value in R into A and set R to 1.0
			*pdwSwizzle = ( ( *pdwSwizzle ) & 0x0000ffff ) | ( ( ( ( *pdwSwizzle ) & 0x00ff0000 ) << 8 ) | 0x00ff0000 );

			// Move value in R into A and set R and B to 0.0
			//*pdwSwizzle = ( ( *pdwSwizzle ) & 0x0000ff00 ) | ( ( ( *pdwSwizzle ) & 0x00ff0000 ) << 8 );
	...
	psurfSwizzle->UnlockRect();

	// Copy psurfSwizzle back into real surface
	hr = D3DXLoadSurfaceFromSurface(psurfOrig, NULL, NULL, psurfSwizzle, NULL, NULL, D3DX_DEFAULT, 0);

	// Release allocated interfaces
	ReleasePpo(&psurfSwizzle);
	ReleasePpo(&psurfOrig);
