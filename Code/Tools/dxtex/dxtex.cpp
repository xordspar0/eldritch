...
CDxtexCommandLineInfo::CDxtexCommandLineInfo(VOID)
{
    ...
    m_bMipMap = FALSE;
	m_bNormalHack = FALSE;	// DLP extension
}
...
{   
	...
	// DLP extension: -n to swizzle R into A for DXT5 normal maps
	else if (bFlag && tolower(pszParam[0]) == 'n')
	{
		m_bNormalHack = TRUE;
	}
...
BOOL CDxtexApp::InitInstance()
{
	...
	// DLP extension
	if (cmdInfo.m_bNormalHack && cmdInfo.m_fmt == D3DFMT_DXT5)
	{
		if (pdoc != NULL )
		{
			pdoc->DoNormalHack();
